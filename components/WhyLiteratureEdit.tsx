...
export default function WhyLiteratureEdit() {
  ...
  return (
    <section className="w-full bg-[#FCFAF7] border-y border-[#D1C2B0]/30 py-20 px-6 sm:px-12">
          ...
          {points.map((pt, index) => (
            <div
              ...
              className={`flex flex-col items-center text-center px-4 py-6 group ${
                index < 3
                  ? "lg:border-r lg:border-[#D1C2B0]/20"
                  : ""
              } ${
                index % 2 === 0 ? "md:border-r-0 lg:border-r lg:border-[#D1C2B0]/20" : ""
              }`}
            >
              {/* Icon */}
              <div className="mb-5 text-[#963D26] opacity-90 transition-transform duration-300 group-hover:scale-105">
                {pt.icon}
              </div>

              {/* Title */}
              <h3 className="font-display text-[11px] font-bold tracking-[0.18em] text-[#963D26] mb-3">
                {pt.title}
              </h3>

              {/* Description */}
              <p className="font-serif-body text-[13px] leading-[1.6] text-[#2A2421]/80 max-w-[220px]">
                {pt.description}
              </p>
            </div>
          ))}
