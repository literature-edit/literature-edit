...
export default function FeaturedProgrammes() {
  const programmes = [
    {
      ...
      icon: (
        <svg
          className="w-10 h-10 text-[#963D26]"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.25"
  ...
  return (
    <section id="programmes" className="w-full bg-[#FAF6F0] py-20 px-6 sm:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border-y border-[#D1C2B0]/30 py-8 lg:py-4">
        {programmes.map((prog, index) => (
          <div
            ...
            className={`flex flex-col items-center text-center px-6 py-10 lg:py-8 group transition-all duration-300 hover:bg-[#FCFAF7] ${
              ...
              index < 3
                ? "lg:border-r lg:border-[#D1C2B0]/30 border-b border-[#D1C2B0]/20 md:border-b-0"
                : ""
            } ${
              // Grid dividers for tablet (2x2 structure)
              index % 2 === 0
                ? "md:border-r md:border-[#D1C2B0]/30"
                : ""
            } ${
              // Bottom border for mobile/tablet rows
              index < 2
                ? "border-b border-[#D1C2B0]/30 md:border-b md:border-b-[#D1C2B0]/30 lg:border-b-0"
                : "border-b border-[#D1C2B0]/20 md:border-b-0"
            }`}
          >
            {/* Styled Icon Wrapper */}
            <div className="mb-6 p-4 rounded-full bg-[#FCFAF7] border border-[#D1C2B0]/20 group-hover:border-[#963D26]/40 transition-colors duration-300">
              {prog.icon}
            </div>

            {/* Title */}
            <h3 className="font-display text-sm font-semibold tracking-[0.16em] text-[#963D26] min-h-[40px] flex items-center justify-center">
              {prog.title}
            </h3>

            {/* Description */}
            <p className="font-serif-body text-[13px] leading-[1.65] text-[#2A2421]/80 mt-4 mb-8 flex-grow max-w-[240px]">
              {prog.description}
            </p>
            ...
            <a
              href={prog.link}
              className="font-display text-[9px] tracking-[0.2em] font-semibold text-[#2A2421]/90 hover:text-[#963D26] transition-colors group/link uppercase flex items-center gap-1.5"
            >
              Learn More
